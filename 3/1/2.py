...
for i in range(0,len(message)):
	letterIndex = i%5
	letter = message[i]
	if letter in letters[letterIndex]:
		letters[letterIndex][letter] += 1
	else:
		letters[letterIndex][letter] = 1

letterFrequencyPercentages = copy.deepcopy(letters)
# get frequency percentages
total = len(message)/5 # 28
for entry in letters:
	for letter in letters[entry]:
...
a = list(letters[0].keys())
letters[0].pop(a[0])

for row in letters:
	print(letters[row])
for row in letterFrequencies:
	print(letterFrequencies[row])