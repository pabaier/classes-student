...
public class Board {
    ...
    public Iterable<Board> neighbors() {
        ...
        // tl corner (2 boards)
        if (zr == 0 && zc == 0) {
            neigh.add(swap(blocks, 0, 0, 0, 1));
            neigh.add(swap(blocks, 0, 0, 1, 0));
        }
        // tr corner (2 boards)
        else if (zr == 0 && zc == n - 1) {
            neigh.add(swap(blocks, zr, zc, 0, n - 2));
            neigh.add(swap(blocks, zr, zc, 1, n - 1));

        }
        // bl corner (2 boards)
        else if (zr == n - 1 && zc == 0) {
            neigh.add(swap(blocks, zr, zc, n - 2, 0));
            neigh.add(swap(blocks, zr, zc, n - 1, 1));

        }
        // br corner (2 boards)
        else if (zr == n - 1 && zc == n - 1) {
            neigh.add(swap(blocks, zr, zc, n - 2, n - 1));
            neigh.add(swap(blocks, zr, zc, n - 1, n - 2));

        }
        // top edge (3 boards)
        else if (zr == 0) {
            neigh.add(swap(blocks, zr, zc, 0, zc + 1));
            neigh.add(swap(blocks, zr, zc, 0, zc - 1));
            neigh.add(swap(blocks, zr, zc, 1, zc));

        }
        // left edge (3 boards)
        else if (zc == 0) {
            neigh.add(swap(blocks, zr, zc, zr - 1, 0));
            neigh.add(swap(blocks, zr, zc, zr + 1, 0));
            neigh.add(swap(blocks, zr, zc, zr, 1));

        }
        // bottom edge (3 boards)
        else if (zr == n - 1) {
            neigh.add(swap(blocks, zr, zc, zr, zc + 1));
            neigh.add(swap(blocks, zr, zc, zr, zc - 1));
            neigh.add(swap(blocks, zr, zc, zr - 1, zc));
