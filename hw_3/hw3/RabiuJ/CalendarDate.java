...
public class CalendarDate {
    ...
    public String getDayOfWeek() {
        int dayCount = 1;
        CalendarDate hintdate = new CalendarDate(1753, 1, 1);
        while (!hintdate.equals(this)){
            hintdate.nextDay();
            dayCount = (dayCount + 1) % 7;
        }
        return DAY_NAMES[dayCount]; 
    }
    ...
    public void nextDay() {
       day = day + 1;
       if (day > getDaysInMonth()){//helper method
        month = month +1;
        day = 1;
        if(month > 12){
          year = year + 1;
    ...
    //adding method to get days in a month
    private int getDaysInMonth(){
        int thedays = DAYS_PER_MONTH[month];
