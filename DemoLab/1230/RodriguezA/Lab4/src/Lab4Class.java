...
/**
	...
	public static void main(String[] args) {
	  ...
	  double min = minimum(min); // change this to a call to minimum method
	  double max = maximum(max); // change this to a call to maximum method
	  double mean = average(avg); // change this to a call to average method
	  double var = varience(varience); // change this to a call to variance method
		   ...
		   public static double variance (double [] x, double avg){
			   ...
			   for(i=0; i<x.length; i++){
				   if(x[i]<variance){
					variance=1.0/x.length*(x[i])-average(x,avg);   
				   }
			   }
